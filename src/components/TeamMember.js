...
const TeamMember = ({borderColor,bgColor}) => {
  return (
    <div className="relative flex flex-col items-center contactusElement">
      <div className={`border-2 border-${borderColor} rounded-[50%] w-3/4 p-3 `}>
        <img src="/rp1.png" alt="image" className=" mx-auto rounded-full " />
      </div>

      <div className="flex flex-col items-center">
        <div className={`h-[130px] w-[2px] bg-${bgColor} `}> </div>
        ...
        <div className={` bg-${bgColor} w-[20px] h-[20px] rounded-[50%] `}></div>
      </div>
    </div>
  );
};
